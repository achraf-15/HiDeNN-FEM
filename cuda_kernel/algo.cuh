...
__device__ void invert_cholesky(const float* A, float* Ainv, int k) {
    float L[32*32];    // lower-triangular Cholesky factor
    float Y[32*32];    // intermediate solution
    float I[32*32];    // identity matrix
...
// A: input symmetric matrix (k x k) in row-major
// Ainv: output inverse (k x k)
__device__ void invert_ldlt(const float* A, float* Ainv, int k)
{
    // Storage for L (unit lower triangle) and D (diagonal)
    float L[32*32];
    float D[32];
    ...
    // --- Inverse via solving L D L^T x = e_r ---
    float y[32], z[32], x[32];

    for(int r=0; r<k; r++){
        // Solve L y = e_r
        for(int i=0;i<k;i++){
            float rhs = (i == r ? 1.0f : 0.0f);
            float acc = 0.0f;
            for(int j=0;j<i;j++){
                acc += L[i*k + j] * y[j];
            }
...
__device__ void invert_small(float* A, float* Ainv, int n){
    ...
    for(int i=0;i<n;i++){
        ...
        if(max_row != i){
            for(int j=0;j<n;j++){
                float tmp = A[i*n+j];    A[i*n+j] = A[max_row*n+j];    A[max_row*n+j] = tmp;
