...
def Strong_Wolfe(obj_func, current_params, d, t, F_k, g_Ok, gtd, options=None):
    """
    Inputs:
        obj_func (callable): reevaluates model and returns function value, grad and gtd
        current_params (tensor): current parameters
        d (tensor): 1-D tensor specifying search direction
        t (float): initial steplength
        F_k (tensor): current loss evaluation
        g_Ok (tensor): 1-D tensor of flattened gradient over overlap O_k used
                        for gradient differencing in curvature pair update
        gtd (tensor): directional derivative
    ...
    # apply step and evaluate new point  
    F_new, g_new, gtd_new = obj_func(current_params, t, d)      
    closure_eval += 1; grad_eval += 1
    ls_step
    ...
    g_prev_vec = g_Ok # initial gradient vector
...
class FullBatchLBFGS(Optimizer):
    ...
    def _directional_evaluate(self, closure, current_params, t, d):
        # apply step
        ...
        # go back to current_params
        self._load_params(current_params)

        return F_new, g_new, gtd_new
    ...
    def _step(self, closure, p_k, g_Ok, options=None):
        """
        ...
        # objective function to reevaluate the function value, grad and gtd
        def obj_func(x, t, d):
            return self._directional_evaluate(closure, x, t, d)
        
        F_new, g_new, t, ls_step, f_eval, g_eval = Strong_Wolfe(obj_func, current_params, d, t, F_k, g_Ok, gtd, options)
        closure_eval += f_eval
        grad_eval += g_eval
        # ----------------------------------
    ...
    def step(self, closure,  options=None):
        ...
        state = self.state['global_state']
        if state['n_iter'] > 0:
            self.curvature_update(grad, eps)

        # compute search direction
        p = self.two_loop_recursion(-grad)

        # take step
        F_new, g_new, t, ls_step, closure_eval, grad_eval  = self._step(closure, p, grad, options=options)
