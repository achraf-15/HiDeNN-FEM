...
def Strong_Wolfe(obj_func, current_params, d, t, F_k, g_k, gtd, options=None):
    """
    Inputs:
        ...
        g_k (tensor): 1-D tensor of flattened gradient over overlap O_k used
    ...
    g_prev_vec = g_k # initial gradient vector
...
class ConjugateGradient(Optimizer):
    ...
    def step(self, closure,  options=None):
        ...
        if state["n_iter"] == 0:
            t = float(min(1.0, 1.0 / flat_grad.abs().sum()) * lr)
        else:
            t = lr
        ...
        F_new, g_new, t, ls_step, f_eval, g_eval = Strong_Wolfe(obj_func, current_params, d, t, orig_loss, flat_grad, gtd, options)
        closure_eval += f_eval
        grad_eval += g_eval
        # ----------------------------------
